...
本地文件不是第二个可独立修改的主数据源。桌面端首次设置时直接打开原生目录选择器，在用户选定的目录中创建固定名称 JSON，不要求输入路径或手建文件；已有旧版路径仍可继续使用或重新选择目录。EDA 主存储成功后串行刷新该备份，停用状态不会留下阻塞后续启用的已完成队列；宿主临时拒绝或文件写入异常只重试一次。Web 端保留“另存库存备份”以创建独立快照。备份、状态保存或通知失败均不得改变已经成功的主写入结果。完整替换恢复先严格校验，再保存恢复前快照；主恢复写入失败时恢复原有恢复点。自动备份日志只记录状态、revision、尝试次数和失败分类，不记录路径或库存内容。详细边界见[后续功能路线](roadmap.md)。

## 商城商品与 EDA 模型边界

商城商品信息属于库存身份数据。EDA 器件模型是可选的设计期引用，包含系统库 device UUID 和 library UUID。没有 EDA 模型的商城商品仍可保存、计数、标记用完、导入和参与库存搜索；复制到常用库和在原理图中放置则必须具有模型引用。放置意图只从 IFrame 回传库存 ID 与 revision，控制器重新读取最新模型引用，平台适配器确认当前文档为原理图后才向 `eda.sch_PrimitiveComponent.placeComponentWithMouse()` 传入 UUID。

当前只实现系统库查询得到的模型引用；从已放置器件捕获个人库、工程库或外部库引用，以及由插件托管真正的本地模型载荷，均尚未实现。后续接入必须先验证引用跨重启和跨工程的稳定性，具体路线与准入门槛见[本地器件模型接入能力](local-device-model-integration.md)。
...
嘉立创 EDA 专业版 3.2.166 的文件系统 BETA 适配存在三个已实机确认的兼容问题：`getDocumentsPath()` 可能返回空字符串，`createDirectoryInFileSystem()` 与客户端建目录处理器存在 `folderPath/path` 参数错配，扩展沙箱还会把全局 `fetch()` 替换为固定拒绝的禁用桩。自动备份不再依赖前两个返回值推断或创建默认目录，也不直接调用全局 `fetch()`。适配器每次先调用公开 `getDocumentsPath()` 完成桌面端校验，再通过公开且受“允许外部交互”权限控制的 `SYS_ClientUrl.request()` 请求当前客户端已有的 `app://api/client/openDir`，最后用公开 `saveFileToFileSystem()` 把固定 JSON 写入用户选定的绝对目录。取消选择不修改状态，Web、未授权、非 2xx、宿主异常和畸形响应均不会被当成取消或继续写入。

`SYS_ClientUrl.request()` 是公开扩展 API，但 `app://api/client/openDir` 是 3.2.166 的内部桥接。该组合集中封装在平台适配层，业务层只依赖 `selectFolder()` 端口；宿主提供可返回目录绝对路径的公开选择器后，应立即替换并删除内部调用。
